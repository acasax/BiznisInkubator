        ...
        if ($num_of_company > 0) { //Provera da li je iz baze učitan podatak
            if($email == $row["email"]){ //Provera sa kojim podacima iz baze se poklapa
                if(md5($password) == $row["password"]){ //Provera šifre
                    ...
                    $user_class->returnJSON("OK",'Uspesno ste se prijavili');
                    return;
                } else{
                    $user_class->returnJSON('ERROR', "Pogresna sifra!");
                    return;
                }
            } else{
                $user_class->returnJSON('ERROR', "Pogresan e-mail!");
                return;
            }
        } else{
            $user_class->returnJSON('ERROR', "E-mail nije u bazi!");
            return;
        }
